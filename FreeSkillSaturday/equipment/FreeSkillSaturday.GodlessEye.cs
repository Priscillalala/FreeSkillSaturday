...
partial class FreeSkillSaturday
{
    public static class GodlessEye
    {
        ...
        public static void Init(FreeSkillSaturday instance)
        {
            ...
            if (enabled)
            {
                instance.loadStaticContentAsync += CreateEquipmentAsync;
                instance.loadStaticContentAsync += CreateDelayedDeathHandlerAsync;
            }
        }

        private static IEnumerator<float> CreateEquipmentAsync(LoadStaticContentAsyncArgs args)
        {
            var loadOps = (
                matMSObeliskLightning: Addressables.LoadAssetAsync<Material>("RoR2/Base/mysteryspace/matMSObeliskLightning.mat"),
                matMSObeliskHeart: Addressables.LoadAssetAsync<Material>("RoR2/Base/mysteryspace/matMSObeliskHeart.mat"),
                matMSStarsLink: Addressables.LoadAssetAsync<Material>("RoR2/Base/mysteryspace/matMSStarsLink.mat"),
                matJellyfishLightning: Addressables.LoadAssetAsync<Material>("RoR2/Base/Common/VFX/matJellyfishLightning.mat"),
                PickupDeathEye: args.assets.LoadAsync<GameObject>("PickupDeathEye"),
                texDeathEyeIcon: args.assets.LoadAsync<Sprite>("texDeathEyeIcon"),
                texDeathEyeConsumedIcon: args.assets.LoadAsync<Sprite>("texDeathEyeConsumedIcon"),
                texCompleteMultiplayerUnknownEndingIcon: args.assets.LoadAsync<Sprite>("texCompleteMultiplayerUnknownEndingIcon")
            );
            return new GenericLoadingCoroutine
            {
                ...
                delegate
                {
                    GameObject pickupModelPrefab = loadOps.PickupDeathEye.asset;
                    ...
                    sharedMaterials[1] = loadOps.matMSObeliskLightning.Result;
                    ...
                    pickupModelPrefab.transform.Find("EyeBallFX/Weird Sphere").GetComponent<ParticleSystemRenderer>().sharedMaterial = loadOps.matMSObeliskHeart.Result;
                    pickupModelPrefab.transform.Find("EyeBallFX/LongLifeNoiseTrails, Bright").GetComponent<ParticleSystemRenderer>().trailMaterial = loadOps.matMSStarsLink.Result;
                    pickupModelPrefab.transform.Find("EyeBallFX/Lightning").GetComponent<ParticleSystemRenderer>().sharedMaterial = loadOps.matJellyfishLightning.Result;

                    Equipment.DeathEye = args.content.DefineEquipment("DeathEye")
                        .SetIconSprite(loadOps.texDeathEyeIcon.asset)
                    ...
                    Equipment.DeathEyeConsumed = args.content.DefineEquipment("DeathEyeConsumed")
                        .SetIconSprite(loadOps.texDeathEyeConsumedIcon.asset)
                        .SetEquipmentType(EquipmentType.Lunar)
                    ...
                    Achievements.CompleteMultiplayerUnknownEnding = args.content.DefineAchievementForEquipment("CompleteMultiplayerUnknownEnding", Equipment.DeathEye)
                        .SetIconSprite(loadOps.texCompleteMultiplayerUnknownEndingIcon.asset)
                    ...
                    void AddDisplayRules(ItemDisplaySpec itemDisplay)
                    {
        ...
        private static IEnumerator<float> CreateDelayedDeathHandlerAsync(LoadStaticContentAsyncArgs args)
        {
            var MSObelisk = Addressables.LoadAssetAsync<GameObject>("RoR2/Base/mysteryspace/MSObelisk.prefab");
            return new GenericLoadingCoroutine
            {
                new AwaitAssetsCoroutine { MSObelisk },
                delegate
                {
                    ...
                    args.content.networkedObjectPrefabs.Add(DelayedDeathHandler);
                }
            };            
        }
