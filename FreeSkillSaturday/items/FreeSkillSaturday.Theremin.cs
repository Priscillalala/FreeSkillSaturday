...
partial class FreeSkillSaturday
{
    public static class Theremin
    {
        ...
        {
            var loadOps = (
                texThereminIcon: args.assets.LoadAsync<Sprite>("texThereminIcon"),
                PickupTheremin: args.assets.LoadAsync<GameObject>("PickupTheremin"),
                DisplayTheremin: args.assets.LoadAsync<GameObject>("DisplayTheremin")
            );
            return new GenericLoadingCoroutine
            {
                ...
                delegate
                {
                    Items.Theremin = args.content.DefineItem("Theremin")
                        .SetIconSprite(loadOps.texThereminIcon.asset)
                        .SetItemTier(ItemTier.Tier2)
                        .SetPickupModelPrefab(loadOps.PickupTheremin.asset, new ModelPanelParams(new Vector3(56, 180, 0), 1, 5))
                        .SetTags(ItemTag.Damage, ItemTag.InteractableRelated, ItemTag.OnKillEffect);

                    Ivyl.SetupItemDisplay(loadOps.DisplayTheremin.asset);
                    ItemDisplaySpec itemDisplay = new ItemDisplaySpec(Items.Theremin, loadOps.DisplayTheremin.asset);
